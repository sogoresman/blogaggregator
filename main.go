...
func main() {
	...
	db, err := sql.Open("postgres", "postgres://postgres:postgres@localhost:5432/blogator?sslmode=disable")
	if err != nil {
	...
	dbQueries := db.QueryRow("SELECT * from users")

	fmt.Sprintf("S1", dbQueries)
